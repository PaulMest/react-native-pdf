...
export default class Pdf extends Component {
  ...
  _parseLayoutProps = (message) => {
    return {
      width: Number(message[1]),
      height: Number(message[2]),
      x: Number(message[3]),
      y: Number(message[4])
    }
  };
  ...
  _onChange = (event) => {
    ...
    const {
      onLoadComplete, onPageChanged, onError, onScroll, onScrollEnd, onZoomEnd,
    } = this.props;
    ...
    if (message.length > 0) {
      if (message[0] === "loadComplete") {
        onLoadComplete && onLoadComplete(Number(message[1]), this.state.path)
        ...
        onScrollEnd && onScrollEnd(this._parseLayoutProps(message))
...
const PdfCustom = requireNativeComponent("RCTPdf", Pdf, {
